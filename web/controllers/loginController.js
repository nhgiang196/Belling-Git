            ...
            if(!CheckBrowser().IsChrome)
              {
				  ...
				  Notifications.addError({'status': 'error', 'message': 'Please use our system on Google Chorme Browser'});
             ...
             if($scope.isRemember){
                ...
                authapp.login({username:$scope.username,password:$scope.password||""}).$promise.then(function (data) {
                    ...
                    console.log(err);
                        Notifications.addError({'status': 'error', 'message': $scope.username + '\'s account or password is incorrect!'});
