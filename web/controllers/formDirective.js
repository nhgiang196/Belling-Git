...
define(['app', 'bpmn'], function (app, Bpmn) {
    ...
    app.directive('approvalMaster',['LIMSService', 'Auth', '$q','$filter','$routeParams',
    function (LIMSService, Auth, $q,$filter,$routeParams) {
        return {
            restrict: 'E',
            controller: function ($scope) {              
                ...
                LIMSService.ISOQualify.GetDetailReport({
                    ...
                    if (data.length > 0) {
                        ...
                        for (var key in data[0]) {
                            if (['VoucherID', 'VoucherType', 'State', 'VoucherNO',
                                    'CreateDate', 'BeginDate', 'EndDate', 'Stamp', 'Status',
                                    'Reason', 'Solution', 'Prevention', 'CreateBy', 'SampleName', 'LOT_NO'
                    ...
                    if (data[0].VoucherType == 'Red')
                        $scope.color = {
                            'TW': '紅',
                            'VN': 'ĐỎ',
                            'Status':false,
                            'ISO': data[0].VoucherNO
                        };
                    else $scope.color = {
                        'TW': '黃',
                        'VN': 'VÀNG',
                ...
                function saveInitData() {
                    var note = {};
                    note.VoucherID = $scope.recod.comp_id || '';
                    note.Prevention = $scope.recod.comp_name;
                    note.Reason = $scope.recod.comp_code;
                    note.State = $scope.recod.type;
                    note.Solution = $scope.recod.phone;
            ...
            templateUrl: './forms/QualifiedControl/ApprovalPage.html'
        }
    }]);
});