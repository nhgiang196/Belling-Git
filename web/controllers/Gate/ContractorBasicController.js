...
define(['myapp', 'angular'], function (myapp, angular) {
    ...
    myapp.controller("ContractorBasicController", ['$scope', 'EngineApi', '$http', 'EngineApi', 'Notifications', '$upload', '$compile', '$filter', 'Auth', '$resource', 'uiGridConstants', '$location', 'GateGoodsOut', 'Forms', '$translate', 'ContractorInspectService', '$translatePartialLoader',
        function ($scope, EngineApi, $http, EngineApi, Notifications, $upload, $compile, $filter, Auth, $resource, uiGridConstants, $location, GateGoodsOut, Forms, $translate, ContractorInspectService, $translatePartialLoader) {
            var col = [
                        ...
                        ContractorInspectService.delContractorsToDB().del({},bcontent,function(res){
                            if(res){
                                if(res.IsSuccess){
                                    ...
                                    Notifications.addError({'status': 'error', 'message':$translate.instant('Delete_Failed_Msg'),"duration":4000});
                                }
                            }else{
                                Notifications.addError({'status': 'error', 'message':$translate.instant('Delete_Failed_Msg'),"duration":4000});
                            }
            ...
            $scope.addContractor=function(){
                ...
                var bcontent={id:$scope.add_contractorID,name:$scope.add_contractor,type:$scope.add_type,category:$scope.add_category};
                ...
                ContractorInspectService.getContractorsToDB().get({"name":$scope.add_contractor},function(res){
                    ...
                    else
                    {
                        ...
                        ContractorInspectService.addContractorsToDB().add({},bcontent,function(res){
                            if(res){
                                if(res.IsSuccess){
                                    Notifications.addMessage({'status': 'success', 'message':$translate.instant('Save_Success_MSG'),"duration":4000});
                                    //$window.alert("添加成功");
                                }else{
                                    Notifications.addError({'status': 'error', 'message':$translate.instant('Msg_Save'),"duration":4000});
                                }
                            }else{
                                Notifications.addError({'status': 'error', 'message':$translate.instant('Msg_Save'),"duration":4000});
                            }
            ...
            $scope.updateContractor=function(){
                //update_new_location
                var bcontent={keshi:$scope.update_kesi,location:$scope.update_location,newlocation:$scope.update_new_location};
