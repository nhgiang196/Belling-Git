...
define(['myapp', 'angular'], function(myapp, angular) {
    myapp.controller("GatePlanController", ['$rootScope','$scope', '$filter', '$http', '$compile', '$routeParams', '$resource', '$location', '$interval', 'Notifications', 'Forms', 'Auth',
        function ($rootScope,$scope, $filter, $http, $compile, $routeParams, $resource, $location, $interval, Notifications, Forms, Auth) {
            ...
            var PlanCol = [
                {field: 'VoucherID',displayName: '计划号',minWidth: 140},
                {field: 'TransferCompany',displayName: '车行',minWidth: 80},
                {field: 'VehicleNO',displayName: '车号',minWidth: 80},
                {field: 'Customer',displayName: '客户',minWidth: 80},
                {field: 'Driver',displayName: '司机',minWidth: 80,cellTooltip: true},
                {field: 'Definition',displayName: ' 目的地',minWidth: 100,cellTooltip: true},
                {field: 'MaterialType',displayName: '物料类型',minWidth: 140,cellTooltip: true},
                {field: 'EffectiveTo',displayName: ' 有效期',minWidth: 100, cellFilter: 'date: "yyyy-MM-dd"',cellTooltip: true},
                {field: 'Remark',displayName: '备注',minWidth: 150,cellTooltip: true},
                {field: 'CloseTime',displayName: '关闭日期',minWidth: 150, cellFilter: 'date: "yyyy-MM-dd"',cellTooltip: true},
                {field: 'CloseUser',displayName: ' 关闭人',minWidth: 100,cellTooltip: true},
                {field: 'CloseRemark',displayName: ' 关闭原因',minWidth: 150,cellTooltip: true},
                {field: 'Stamp',displayName: '创建日期',minWidth: 150, cellFilter: 'date: "yyyy-MM-dd"',cellTooltip: true}
            ];
            ...
            function SaveShortPlan(plans){
                console.log(plans);
                GateEngine.ShortTruckPlan().save({"Recode":plans,"Createdate": $filter('date')(new Date(), "yyyyMMdd")}, function (message) {
                    //console.log(message);
                    getvoucherid(message,function (voucherid) {
                        console.log("------------"+voucherid);
                        console.log(voucherid.length);
            ...
            $scope.addsaveCQ = function () {
                ...
                GateEngine.ShortTruckPlanItem().save({"Recode":projects,"VoucherID": $scope.note.VoucherID}, function (message) {
                    if (!message) {
