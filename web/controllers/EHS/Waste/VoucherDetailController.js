        ...
        function ($filter, Notifications, Auth, EngineApi, VoucherService, $translate, $q, $scope, $routeParams) {
            ...
            function loadHeader() {
                ...
                VoucherService.GetVoucher({ VoucherID: $scope.VoucherID }, function (data) {
                    ...
                    $scope.date1 = moment(data[0].CreateTime).format('DD/MM/YYYY');
                    $scope.date2 = 'ngày '+ moment(data[0].CreateTime).format("DD") + ' tháng ' +moment(data[0].DateOut).format("MM")+ ' năm '+ moment(data[0].DateOut).format("YYYY");
                }, function (error) {
