...
exports.MyQueryproxyUser=function(req,res){
    var userid=req.query.userid;
    sworm.db(conn).then(function (db) {
        db.query('Select *  from ProxyUser where UserId=@UserId', {UserId: userid})
            .then(function (result) {
                res.send(200, result);
            })
            .catch(function (error) {
...
exports.QueryproxyUser=function(req,res){
    var userid=req.query.userid;
    sworm.db(conn).then(function (db) {
        db.query('Select *  from ProxyUser where ProxyUserId=@ProxyUserId', {ProxyUserId: userid})
            .then(function (result) {
                res.send(200, result);
            })
            .catch(function (error) {
...
exports.QueryproxyUserByName=function(req,res){
    console.log(req.query.userid)
    var userid=req.query.userid;
    var name =req.query.ProcessName;
    sworm.db(conn).then(function (db) {
        db.query('Select *  from ProxyUser where UserId=@UserId and Name = @Name', {UserId: userid,Name:name})
            .then(function (result) {
                res.send(200, result);
                console.log(res)
            })
...
//取消代理人
exports.DeleteProxyUser=function(req,res){
    var userid=req.session['username'];
    var wf=req.query.wfname||"";
    if(userid) {
        sworm.db(conn).then(function (db) {

            if(wf){
                db.query('delete from ProxyUser where UserId=@UserId  and  Name=@Name ', {UserId: userid,Name:wf})
                    .then(function (qresult) {
                        res.send(200);
                    })
                    .catch(function (error) {
                ...
                db.query('delete from ProxyUser where UserId=@UserId', {UserId: userid})
                    .then(function (qresult) {
                        res.send(200);
                    })
                    .catch(function (error) {
...
//设置代理人
exports.AddProxyUser=function(req,res){
    var userid=req.session['username'];
    //增加代理人 ,不能自己设置自己的代理人
    var record = req.body.record;
    sworm.db(conn).then(function (db) {
        var opear = {};
        if (userid) {
            async.eachSeries(record,function(item,callback){
               console.log(item);
                if(item.UserId!=item.ProxyUserId) {
                    opear.saved = false;
                    var ProxyUser = db.model({table: 'ProxyUser', id: ['UserId', 'ProxyUserId', 'Name']});
                    var postion = ProxyUser(item, opear).save({force: true}).then(function () {
                        callback("");
                    }).catch(function (error) {
                        return callback(error);
                    })
                }else{
                    callback("proxy user can not myslet!");
                }
